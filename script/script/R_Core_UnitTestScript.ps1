...
========================================================================================
 ...
 How to Run (Examples)
   MAKE:
     .\run.ps1 --app `
       "C:\src\proj" `
       "C:\cfg\case_player.vopt" `
       "C:\cfg\source_list.txt" `
       "C:\cfg\spmc_dir_list.csv" `
       make `
       "C:\caseviewer\proj_caseviewer\proj\build" `
       "demo_func1.csv" `
       "C:\cfg\AmsProjSave.amso"
 ...
 Exit Codes
   0  : success
   1+ : a specific step failed (message will indicate which; Case/SST hints included)
...
function Ensure-GaioLicense {
    param(
        [string]$LicenseServer = "100.64.4.135",
        [int]$Port = 50001,
        [string]$GaioBinPath   # optional: folder containing en_GaioLicClient.exe
    )
...
function Install-WinAMS {
  ...
  $setup = Get-ChildItem $temp -Recurse -Filter *.exe | Select-Object -First 1
  if (-not $setup) {
...
# --- 2) Install WinAMS if not already installed ---
try {
  Install-WinAMS
} catch {
...
try {
  Ensure-GaioLicense -LicenseServer "100.64.4.135" -Port 50001
} catch {
